...
def _job_intraday_monitor(*, ignore_cutoff: bool = False) -> None:
    """
    Run one intraday portfolio monitor pass with the same 13:30 cutoff as jobs.py.

    僅負責「持股盤中監控」。N 字底突破偵測已移至 scheduler/jobs.py 的
    job_intraday_monitor（隨 srock up 啟動，無需瀏覽器訪問），不在此處執行。
    """
