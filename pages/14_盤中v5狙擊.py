...
組件：
  ...
  2. 今日 watchlist：display today_all()（含已推播）→ 表格 + 型態欄位
  3. 手動工具：立即重建 watchlist（含糾結度 / 漲幅 cap 參數）/ 立即跑一次盤中檢查
  4. 突破警示歷史：從 event_log 撈 v5_sniper_alert*
"""
...
st.caption(
    "每日 08:50 自動建構科技股 v5 候選清單（型態 A：糾結突破 / 型態 B：N 字底再上），"
    "9:00–13:30 每分鐘檢查突破目標價 + 量增 1.5× 並推播 Telegram。"
...
if not items:
    ...
    cm2.metric("型態 A", n_a)
    cm3.metric("型態 B", n_b)
    cm4.metric("已推播", alerted)
    cm5.metric("距目標 ≤ 2%", near)
...
with mc2:
    ...
    st.caption(
        "對今日 watchlist 中尚未推播的候選跑一次 Shioaji 報價 + 突破/量增/漲幅判斷。"
        "Shioaji 未登入時整輪會安靜略過。"
    )
