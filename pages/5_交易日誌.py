...
from modules.journal import add_trade, get_all_trades, delete_trade, calc_performance, calc_emotion_stats
...
with tab_stats:
    ...
    if not perf:
        ...
        c3.metric("盈虧比", perf["profit_factor"],
                  delta="良好" if perf["profit_factor"] >= 1.5 else "需改善")
        c4.metric("總損益", f"{perf['total_pnl']:+,.0f} 元",
                  delta_color="normal")

        c5, c6, c7, c8 = st.columns(4)
        c5.metric("平均獲利", f"{perf['avg_win']:+,.0f} 元")
        c6.metric("平均虧損", f"{perf['avg_loss']:+,.0f} 元")
...
with tab_log:
    ...
    if df.empty:
        ...
        display = filtered.rename(columns={
            "stock_id": "代碼", "stock_name": "名稱", "action": "動作",
            "price": "價格", "shares": "張數", "trade_date": "日期",
        ...
        # 刪除
        st.markdown("---")
        del_id = st.number_input("輸入要刪除的記錄 ID", min_value=1, step=1, value=1)
        if st.button("刪除此記錄", type="secondary"):
            all_ids = df["id"].tolist()
            if del_id in all_ids:
                delete_trade(del_id)
...
with tab_add:
    ...
    with st.form("add_trade_form", clear_on_submit=True):
        ...
        with r1c3:
            ...
            t_shares = st.number_input("張數 *", min_value=1, value=1, step=1)
